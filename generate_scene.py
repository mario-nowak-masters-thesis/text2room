...
def main(args):
    ...
    if not args.quick_run: 
        save_poisson_mesh(intermediate_mesh_path, depth=args.poisson_depth, max_faces=args.max_faces_for_poisson) # ! this takes 1.5 hours

        # run completion
        pipeline.args.update_mask_after_improvement = True
        pipeline.complete_mesh(offset=offset) # ! this takes 1.25 hours
        pipeline.clean_mesh()
